...
export function ImageNavigation({ authKey, conversation, viewport, onLocate, open, onOpenChange }: {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      ...
      setActive((previous) => {
        const turn = candidates[0].closest<HTMLElement>("[data-turn-id]");
        if (!turn) return previous;
        const next = { turn_id: turn.dataset.turnId! };
        return previous?.turn_id === next.turn_id ? previous : next;
      });
  ...
  const tree = <nav aria-label="结果定位导航" className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-1 pb-2 text-xs">
    ...
    {groups.map((source) => {
      ...
      const label = [source.documentId, source.name.trim()].filter(Boolean).join(" · ");
      return <div key={source.id} className="mb-0.5" data-navigation-source={source.id}>
        <div className={`flex items-center rounded-md ${current ? "bg-emerald-50 text-emerald-800" : "text-stone-700"}`}>
          <button type="button" aria-label={`展开或收起 ${label}`} aria-expanded={expanded.has(source.id)} onClick={() => toggle(source.id)} className="flex size-7 shrink-0 items-center justify-center rounded">
            <ChevronRight className={`size-3.5 ${expanded.has(source.id) ? "rotate-90" : ""}`} />
          </button>
          <button type="button" title={label || "普通生成"} className="flex min-h-8 min-w-0 flex-1 items-center gap-1 pr-2 text-left" aria-current={current ? "location" : undefined}
            onClick={() => void locate({ turn_id: source.turns.at(-1)!.id })}>
            <span className="min-w-0 flex-1 truncate">{label || "普通生成"}</span>
            {current && <span className="shrink-0 text-[10px]" aria-label={`当前位置，第 ${current.sourceOrdinal} 次`}>第 {current.sourceOrdinal} 次</span>}
          </button>
        </div>
        {expanded.has(source.id) && source.turns.map((turn) => <div key={turn.id} className="ml-3 border-l border-stone-200 pl-2">
          <button type="button" className="min-h-7 w-full rounded px-2 py-1 text-left aria-[current=location]:bg-emerald-100" aria-current={active?.turn_id === turn.id ? "location" : undefined}
  ...
  return <>
    <aside className="hidden h-full min-h-0 min-w-0 flex-col border-l border-stone-200 pl-1 xl:flex">
      <h2 className="px-3 py-2 text-sm font-medium">结果定位</h2>{tree}
    </aside>
