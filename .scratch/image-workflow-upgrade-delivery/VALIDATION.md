...
十八项全部已集成并验证。完整离线340 passed、7 subtests passed（131.39秒，隔离树data/final-offline.log）；原5项基线失败已由04/05关闭。此后应用修改仅为前端，最终83a872a独立TypeScript检查与11页生产构建通过。

最终浏览器包含两种3200实际PNG的完整分页、混合来源导航、轮询刷新后定位、手动滚动、双浏览器恢复及320×360操作；12的98个实际独立原图下载逐张哈希一致；18的三服务器时区午夜与三浏览器时区通过。各场景所用提交、命令、指标和证据见12/14/18及其他逐票报告。上述真实服务手动联调仍按授权边界未运行，没有将受控上游结果称为真实上游验收。
