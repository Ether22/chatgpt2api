"""Serve ticket 07's production page with real temporary auth/task/image storage.
...
os.environ["CHATGPT2API_AUTH_KEY"] = "ticket07-bootstrap-only"
...
if __name__ == "__main__":
    ...
    temporary_parent = ROOT / "data" / "ticket07"
    ...
    with tempfile.TemporaryDirectory(dir=temporary_parent) as temporary:
        ...
        for name in ("A", "B"):
            ...
            auth.update_key(identity["id"], {"key": f"ticket07-{name}"})
        ...
        app = FastAPI()
        app.include_router(system.create_router("ticket07-test"))
        app.include_router(accounts.create_router())
        app.include_router(ai.create_router())
        app.include_router(image_tasks.create_router())
        app.include_router(image_imports.create_router())
        @app.get("/ticket07-consumption")
        def consumption():
