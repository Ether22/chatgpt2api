"""Serve ticket 03's production page with real temporary auth/task/image storage.
...
os.environ["CHATGPT2API_AUTH_KEY"] = "ticket03-bootstrap-only"
...
if __name__ == "__main__":
    ...
    temporary_parent = ROOT / "data" / "ticket03"
    ...
    with tempfile.TemporaryDirectory(dir=temporary_parent) as temporary:
        ...
        for name in ("A", "B"):
            ...
            auth.update_key(identity["id"], {"key": f"ticket03-{name}"})
        ...
        app = FastAPI()
        app.include_router(system.create_router("ticket03-test"))
