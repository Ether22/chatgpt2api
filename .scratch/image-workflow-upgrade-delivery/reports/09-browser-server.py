"""Serve ticket 09's production page with real temporary auth/task/image storage.
...
os.environ["CHATGPT2API_AUTH_KEY"] = "ticket09-bootstrap-only"
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory(dir=temporary_parent) as temporary:
        ...
        for name in ("A", "B"):
            ...
            auth.update_key(identity["id"], {"key": f"ticket09-{name}"})
        ...
        app = FastAPI()
        app.include_router(system.create_router("ticket09-test"))
