...
os.environ["CHATGPT2API_AUTH_KEY"] = "ticket06-bootstrap-only"
...
if __name__ == "__main__":
    ...
    temporary_parent = ROOT / "data" / "ticket06"
    ...
    with tempfile.TemporaryDirectory(dir=temporary_parent) as temporary:
        ...
        for name in ("A", "B"):
            ...
            auth.update_key(identity["id"], {"key": f"ticket06-{name}"})
        ...
        app = FastAPI()
        app.include_router(system.create_router("ticket06-test"))
        app.include_router(accounts.create_router())
        app.include_router(ai.create_router())
        app.include_router(image_tasks.create_router())
        app.include_router(image_imports.create_router())
        @app.get("/ticket06-consumption")
        def consumption():
        ...
        with patch("services.openai_backend_api.OpenAIBackendAPI.list_models", return_value={"data": [{"id": "gpt-image-2"}]}):
            uvicorn.run(app, host="127.0.0.1", port=43160)
