...
(async () => {
  ...
  try {
    for (const timezoneId of ['UTC', 'America/Los_Angeles', 'Asia/Shanghai']) {
      ...
      assert.match(keyText, /创建时间 2026\/09\/08 00:00:01 北京时间/);
      assert.match(keyText, /最近使用 2026\/09\/08 00:00:01 北京时间/);
      assert.match(keyText, /最近使用 2026\/09\/07 23:59:59 北京时间/);
      ...
      assert.match(backupText, /2026\/09\/08 00:00:01 北京时间/);
      ...
      await dialog.getByText('2026/09/07 23:59:59 北京时间', { exact: true }).waitFor();
      await dialog.getByText('2026/09/08 00:00:01 北京时间', { exact: true }).waitFor();
      await page.screenshot({ path: path.join(evidence, timezoneId.replaceAll('/', '-') + '-logs.png') });
      results.push({ timezoneId, key: '2026/09/08 00:00:01 北京时间', backup: '2026/09/08 00:00:01 北京时间', logStart: '2026/09/07 23:59:59 北京时间', logEnd: '2026/09/08 00:00:01 北京时间' });
      await context.close();
    }
  } finally {
    await browser.close();
