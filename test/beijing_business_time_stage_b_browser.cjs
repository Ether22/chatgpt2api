...
(async () => {
  ...
  try {
    for (const timezoneId of ['UTC', 'America/Los_Angeles', 'Asia/Shanghai']) {
      ...
      await row.waitFor();
      assert.match(await row.innerText(), /2026\/09\/08 00:00:01/);
      assert.match(await row.innerText(), /2026\/09\/07 23:59:59/);
      ...
      await page.getByText('2026/09/08 00:00:01', { exact: true }).first().waitFor();
